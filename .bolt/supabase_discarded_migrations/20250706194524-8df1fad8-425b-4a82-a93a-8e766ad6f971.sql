-- Create new role enum with required values
...
-- Add the new role column to profiles table
ALTER TABLE profiles ADD COLUMN user_role user_role DEFAULT 'space_manager';
...
CREATE POLICY "Owners and managers can manage teachers" ON public.teachers
  ...
  USING (
    EXISTS (
      ...
      AND profiles.user_role IN ('owner', 'manager')
    )
  )
  WITH CHECK (
    EXISTS (
