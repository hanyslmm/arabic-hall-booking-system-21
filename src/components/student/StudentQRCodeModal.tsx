...
export const StudentQRCodeModal = ({
  isOpen,
  onClose,
  student,
  ...
  const handlePrintBarcode = () => {
    ...
    // --- NEW FIX ---
    // Match the canvas to the printer's likely physical resolution (DPI).
    // A 50mm x 25mm label at a standard 203 DPI is approx. 400x200 pixels.
    // Generating the barcode at this native size prevents bad scaling by the browser,
    // which caused the squished/blurry result.
    const canvas = document.createElement('canvas')
    canvas.width = 400 // Approx. 50mm at 203 DPI
    canvas.height = 200 // Approx. 25mm at 203 DPI
    ...
    if (JsBarcode) {
      try {
        JsBarcode(canvas, student.serial_number, {
          ...
          lineColor: '#000000',
          background: '#ffffff',
          width: 2, // Use thinner bars (2px) for clarity on a 400px canvas.
          height: 160, // Adjust height to fit the new 200px canvas height.
          displayValue: false,
          margin: 10,
        })
      } catch (e) {
        ...
        printWindow.close()
        alert(
          'Failed to generate barcode. The serial number might contain invalid characters.',
        )
        return
      }
    } else {
    ...
    // This HTML and CSS is correct. It sets the page size and forces the image to fill it.
    const printContent = `
      <!DOCTYPE html>
      <html>
      <head>
